...
namespace HiddenResidue.Player
{
    ...
    public class PlayerMovement : MonoBehaviour
    {
        ...
        [SerializeField] private float footstepInterval = 0.45f; // Jeda waktu antar bunyi langkah (detik)
        ...
        {
            // Jika pemain sedang bergerak dan game dalam keadaan 'Playing'
            if (canMove && moveInput.sqrMagnitude > 0.01f)
            {
                ...
                if (footstepTimer <= 0)
                {
                    // Memanggil SFX Footstep dari AudioManager
                    if (Core.AudioManager.Instance != null)
                    {
                        Core.AudioManager.Instance.PlaySFX(Core.AudioManager.SFX.Footstep);
                    }
                    
                    // Reset timer ke interval yang ditentukan
                    footstepTimer = footstepInterval;
                }
            }
            else
            {
                // Reset timer ke 0 agar saat mulai jalan kembali, suara langsung terdengar
                footstepTimer = 0f;
            }
        }
        ...
        private void UpdateScaleByY()
        {
            ...
            if (ignoreScaleObjects != null && ignoreBaseScales != null)
            {
                for (int i = 0; i < ignoreScaleObjects.Length; i++)
                {
                    ...
                    ignoreScaleObjects[i].localScale = ignoreBaseScales[i] / multiplier;
                }
            }
        }
        ...
        private void HandleGameStateChanged(Core.GameManager.GameState state)
        {
            ...
            if (!canMove)
            {
                ...
                // Reset footstep saat game di-pause atau berhenti
                footstepTimer = 0f;
            }
        }
